...
public class ReturnMainFragment extends android.support.v4.app.Fragment{
    ...
    private int fragmentId = R.layout.fragment_return;
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        listViewReturns = view.findViewById(R.id.listview_return);
        if(mainActivity.status.currentTicket.length()>0){
            orders = createTicket(mainActivity.status.currentTicket);
            ReturnAdapter adapter = new ReturnAdapter(getContext(), mainActivity, orders,true);
            mainActivity.runOnUiThread(()->listViewReturns.setAdapter(adapter));
        }
        view.findViewById(R.id.button_test_ticket).setOnClickListener(v -> {
            orders = createTicket("default");
            ReturnAdapter adapter = new ReturnAdapter(getContext(), mainActivity, orders,true);
            mainActivity.goToBookmarkInTopic("ticketfound","returnmain");
            mainActivity.runOnUiThread(()->listViewReturns.setAdapter(adapter));
        });

        view.findViewById(R.id.button_home_return).setOnClickListener(v -> mainActivity.setFragment(new MainMenuFragment()));
    ...
    public ArrayList<Item> createTicket(String ticketName){
        ...
        for(Item ri: listViewItems){
            mainActivity.setDynamicProductName("product_"+ productPosition, ri.getName());
            productPosition+= 1;
        }
    ...
    public void pressListViewButton(int childPosition){
        ...
        mainActivity.runOnUiThread(tmp::performClick);
    }
